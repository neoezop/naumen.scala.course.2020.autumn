object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
                  func: Function2[Vector2D, Vector2D, Double],
    ...
    */
    ...
    def ballVolume(r: Int): Double = 4 / 3 * java.lang.Math.PI * math.pow(r, 3)

    def ballWeight(v: Double, p: Double): Double = p * v

    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        val byWeight = ballsArray map (s => s._1 -> ballWeight(ballVolume(s._2._1), s._2._2))
        val sortedByWeight =  byWeight.toSeq.sortBy(_._2)
        for (ball <- sortedByWeight)
            yield ball._1
    }
}